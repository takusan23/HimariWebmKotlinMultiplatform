...
sealed interface MatroskaId {
    /** ID のバイト配列 */
    val idByteArray: ByteArray
}
 ...
 * 未知の ID
 *
 * @param idByteArray [SpecMatroskaId]では見つけられなかったデータ
 */
data class UnknownMatroskaId(override val idByteArray: ByteArray) : MatroskaId {
    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (other == null || this::class != other::class) return false

        other as UnknownMatroskaId

        if (!idByteArray.contentEquals(other.idByteArray)) return false

        return true
    }

    override fun hashCode(): Int {
        return idByteArray.contentHashCode()
    }
}


/**
 * 仕様書に存在する Matroska のタグ
 *
 * @param idByteArray ID のバイト配列
 * @param isParent 子要素を持つ親かどうか
 * @param parentTag 親要素の [MatroskaId]
 */
enum class SpecMatroskaId(
    override val idByteArray: ByteArray,
    val isParent: Boolean,
    val parentTag: SpecMatroskaId?
) : MatroskaId {
    ...
    VideoTrack(byteArrayOf(0xE0.toByte()), true, TrackEntry),
